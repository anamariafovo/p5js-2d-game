...
function draw()
{
    ...
    fill(255);
    textSize(18);
    text("Move - W A S D", width - 200, 50);
    ...
    if(lives == 0)
    {
        fill(255);
        textSize(40);
        text("Game Over. Press Space to continue.", width/2 - 250, height/2 - 150);
        return;
    }
    ...
    else if(portal.isReached)
    {
        fill(255);
        textSize(40);
        text("Level Complete. Press Space to continue.", width/2 - 250, height/2 - 150);
        return;
    }
...
function startGame()
{
    ...
    trees_x = [100, 500, 800, 1300, 1550, 2300, 2600, 2790, 3500, 4055, 4500, 4700];
...
function drawBuildings()
{
    ...
    for(var i = 0; i < buildings.length; i++){
        ...
        rect(buildings[i].x_pos+80, buildings[i].y_pos+25,
             buildings[i].width, buildings[i].height-25,
             10, 10, 0, 0
            );
        fill(230);
        //first building in the buildings group
        rect(buildings[i].x_pos-50, buildings[i].y_pos,
             buildings[i].width, buildings[i].height,
             10, 10, 0, 0
            );
        //second building in the buildings group
        rect(buildings[i].x_pos+25, buildings[i].y_pos-100,
             buildings[i].width, buildings[i].height+100,
             10, 10, 0, 0
            );
        //third bulding in the building group
        rect(buildings[i].x_pos+150, buildings[i].y_pos-32,
             buildings[i].width, buildings[i].height+32,
             10, 10, 0, 0
            );
        //the light that reflects from the buildings
        fill(250);
        rect(buildings[i].x_pos-50, buildings[i].y_pos,
             buildings[i].width-75, buildings[i].height,
             10, 0
            );
        rect(buildings[i].x_pos+25, buildings[i].y_pos-100,
             buildings[i].width-75, buildings[i].height+100,
             10, 0
            );
        rect(buildings[i].x_pos+150, buildings[i].y_pos-32,
             buildings[i].width-75, buildings[i].height+32,
             10, 0
            );
    }
}
...
function drawTrees()
{
    ...
    for(var i = 0; i < trees_x.length; i++){
        ...
        strokeWeight(10);  
        //tree trunk
        line(trees_x[i], floorPos_y, 
             trees_x[i], floorPos_y-90
            );
        //tree branches
        line(trees_x[i], floorPos_y-90, 
             trees_x[i]-50, floorPos_y-90
            );
        line(trees_x[i]-50, floorPos_y-90, 
             trees_x[i]-50, floorPos_y-110
            );
        line(trees_x[i], floorPos_y-90, 
             trees_x[i]+30, floorPos_y-90
            );
        line(trees_x[i]+30, floorPos_y-90, 
             trees_x[i]+30, floorPos_y-110
            );
        noStroke();
        fill(102, 187, 106);
        //tree leaves
...
function drawCanyon(t_canyon)
{
    ...
    rect(t_canyon.x_pos, floorPos_y, t_canyon.width, floorPos_y + height - floorPos_y)
    stroke(225);
    strokeWeight(5);
    //canyon margins
    //the line's length is calculated always based on the ground's height (height - floorPos_y)
    line(t_canyon.x_pos, floorPos_y, t_canyon.x_pos, floorPos_y + height - floorPos_y);
    line(t_canyon.x_pos+t_canyon.width, floorPos_y, t_canyon.x_pos+t_canyon.width, floorPos_y + height - floorPos_y);
    noStroke();
}
...
function checkCanyon(t_canyon)
{
    ...
    if(gameChar_x > t_canyon.x_pos+25 && gameChar_x < t_canyon.x_pos+70 && gameChar_y >= floorPos_y){
        isPlummeting = true;
...
function Enemy(x, y, range)
{
    ...
    {   
        ...
        if(isUnder && isRight){
            ...
            this.upperEllipseX = constrain(this.upperEllipseX, this.x-20, this.x+20);

            this.lowerEllipseX -= 10;
            this.lowerEllipseX = constrain(this.lowerEllipseX, this.x-20, this.x+20);
        } 
